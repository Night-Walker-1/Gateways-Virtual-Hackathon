...
describe('GroqAIService', () => {
    ...
    beforeEach(() => {
        ...
        groqService = new groq_ai_service_1.GroqAIService();
        mockGroqClient = groqService.groqClient;
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
    describe('Risk Assessment', () => {
        test('should identify high risk messages', () => {
            ...
            highRiskMessages.forEach(message => {
                const riskLevel = groqService.assessRiskLevel(message);
        ...
        test('should consider sentiment score in risk assessment', () => {
            ...
            const highRisk = groqService.assessRiskLevel(message, -0.8);
            expect(highRisk).toBe('medium');
            // Neutral sentiment should remain low
            const lowRisk = groqService.assessRiskLevel(message, 0.1);
    ...
    describe('System Prompt Building', () => {
        test('should build basic system prompt', () => {
            const userContext = {};
            const riskLevel = 'low';
            const prompt = groqService.buildSystemPrompt(userContext, riskLevel);
    ...
    describe('Conversation History Building', () => {
        test('should handle empty conversation history', () => {
            const userContext = {};
            const history = groqService.buildConversationHistory(userContext);
    ...
    describe('Response Generation', () => {
        test('should generate response successfully', async () => {
            ...
            const response = await groqService.generateResponse('I feel really sad', 'user123', { currentMood: 'sad' }, -0.5);
        ...
        test('should handle API errors with fallback response', async () => {
            ...
            const response = await groqService.generateResponse('I feel sad', 'user123', {}, -0.3);
        ...
        test('should provide crisis response for high risk situations', async () => {
            mockGroqClient.chat.completions.create.mockRejectedValue(new Error('API Error'));
            const response = await groqService.generateResponse('I want to kill myself', 'user123', {}, -0.9);
    ...
    describe('Response Analysis', () => {
        test('should detect intervention types correctly', () => {
            ...
            const crisisMetadata = groqService.analyzeResponse(crisisResponse, 'high', 100);
            expect(crisisMetadata.intervention).toBe('crisis_intervention');
            const anxietyMetadata = groqService.analyzeResponse(anxietyResponse, 'low', 100);
            expect(anxietyMetadata.intervention).toBe('anxiety_management');
            const depressionMetadata = groqService.analyzeResponse(depressionResponse, 'low', 100);
            expect(depressionMetadata.intervention).toBe('depression_support');
            const relationshipMetadata = groqService.analyzeResponse(relationshipResponse, 'low', 100);
        ...
        test('should detect personality modes correctly', () => {
            ...
            const empatheticMetadata = groqService.analyzeResponse(empatheticResponse, 'low', 100);
            expect(empatheticMetadata.personality).toBe('empathetic');
            const practicalMetadata = groqService.analyzeResponse(practicalResponse, 'low', 100);
            expect(practicalMetadata.personality).toBe('practical');
            const mindfulMetadata = groqService.analyzeResponse(mindfulResponse, 'low', 100);
            expect(mindfulMetadata.personality).toBe('mindful');
            const wiseMetadata = groqService.analyzeResponse(wiseResponse, 'low', 100);
    ...
    describe('Follow-up Questions and Actions', () => {
        test('should generate appropriate follow-up questions for crisis intervention', () => {
            const questions = groqService.generateFollowUpQuestions('crisis_intervention');
        ...
        test('should generate appropriate actions for different risk levels', () => {
            const highRiskActions = groqService.generateSuggestedActions('general_support', 'high');
            expect(highRiskActions).toContain('Call 988 immediately');
            const mediumRiskActions = groqService.generateSuggestedActions('general_support', 'medium');
            expect(mediumRiskActions).toContain('Call crisis hotline');
            const lowRiskActions = groqService.generateSuggestedActions('anxiety_management', 'low');
    ...
    describe('Helper Function', () => {
        test('generateGroqResponse should work correctly', async () => {
            ...
            mockGroqClient.chat.completions.create.mockResolvedValue(mockCompletion);
            const response = await (0, groq_ai_service_1.generateGroqResponse)('I need help', 'user123', { currentMood: 'neutral' }, 0.1);
            expect(response.response).toBe('I\'m here to help you through this.');
            expect(response.metadata).toBeDefined();
        });
    });
});
//# sourceMappingURL=groq-ai-service.test.js.map