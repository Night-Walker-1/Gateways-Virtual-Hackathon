...
exports.integrateWithChatController = exports.generateAdvancedAIResponse = void 0;
const advanced_ai_prompt_1 = require("./advanced-ai-prompt");
/**
 ...
 */
const generateAdvancedAIResponse = async (message, userId, userContext, sentimentScore) => {
...
exports.generateAdvancedAIResponse = generateAdvancedAIResponse;
/**
 * STEP 2: OPENAI GPT-4 INTEGRATION (RECOMMENDED)
 *
 * For production-grade AI responses, integrate with OpenAI GPT-4
 */
async function generateOpenAIResponse(message, userId, userContext, riskLevel) {
    // You'll need to install openai: npm install openai
    // const OpenAI = require('openai');
    // const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
    const systemContext = (0, advanced_ai_prompt_1.generateUserContext)(userContext);
    const conversationHistory = userContext.conversationHistory || [];
    const messages = [
        { role: 'system', content: advanced_ai_prompt_1.MINDGUARD_AI_SYSTEM_PROMPT },
 ...
 */
async function generateEnhancedLocalResponse(message, userId, userContext, riskLevel, sentimentScore) {
 ...
 */
function getCrisisResponses(riskLevel) {
    if (riskLevel === 'high') {
        return [
...
function getDepressionResponses(userContext) {
...
function getAnxietyResponses(userContext) {
    const name = userContext.name ? `, ${userContext.name}` : '';
...
function getRelationshipResponses(userContext) {
    const name = userContext.name ? `, ${userContext.name}` : '';
    return [
...
function getWorkStressResponses(userContext) {
    const name = userContext.name ? `, ${userContext.name}` : '';
    return [
 ...
 */
function personalizeResponse(response, userContext) {
...
function getSentimentBasedResponse(message, sentimentScore, userContext, riskLevel) {
    const name = userContext.name ? `, ${userContext.name}` : '';
...
function getAdaptiveDefaultResponse(message, userContext, riskLevel) {
    const name = userContext.name ? `, ${userContext.name}` : '';
    const responses = [
...
function detectInterventionType(response) {
    if (response.includes('988') || response.includes('crisis'))
        return 'crisis_intervention';
    if (response.includes('breathing') || response.includes('ground'))
        return 'anxiety_management';
    if (response.includes('small step') || response.includes('self-care'))
        return 'depression_support';
    if (response.includes('relationship') || response.includes('communication'))
...
function generateSuggestedActions(response, riskLevel) {
    if (riskLevel === 'high') {
 ...
 */
const integrateWithChatController = async (data) => {
    ...
    const aiResponse = await (0, exports.generateAdvancedAIResponse)(data.message, data.userId, userContext, data.sentimentScore);
...
exports.integrateWithChatController = integrateWithChatController;
exports.default = {
    generateAdvancedAIResponse: exports.generateAdvancedAIResponse,
    integrateWithChatController: exports.integrateWithChatController,
    MINDGUARD_AI_SYSTEM_PROMPT: advanced_ai_prompt_1.MINDGUARD_AI_SYSTEM_PROMPT
};
//# sourceMappingURL=advanced-ai-implementation.js.map