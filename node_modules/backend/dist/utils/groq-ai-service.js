...
exports.generateGroqResponse = exports.groqAI = exports.GroqAIService = void 0;
...
class GroqAIService {
    constructor() {
        // Initialize Groq client with API key at runtime
        const apiKey = process.env.GROQ_API_KEY;
        this.groqClient = new Groq({
            apiKey: apiKey || 'your-groq-api-key-here',
        });
        // Validate API key on initialization
        if (!apiKey || apiKey === 'your-groq-api-key-here') {
            console.warn('⚠️  Groq API key not properly configured.');
        }
        else {
     ...
     */
    async generateResponse(message, userId, userContext = {}, sentimentScore) {
        const startTime = Date.now();
        try {
            // Debug: Check API key at runtime
            const currentApiKey = process.env.GROQ_API_KEY;
            console.log(`🔍 Debug - API Key status: ${currentApiKey ? `Present (${currentApiKey.length} chars)` : 'Missing'}`);
            // Detect crisis level
            ...
            const completion = await this.groqClient.chat.completions.create({
                ...
                model: 'llama-3.1-8b-instant', // Fast and effective for conversations
                temperature: 0.7,
                max_tokens: 1000,
                top_p: 0.9,
                stream: false,
            ...
            // Throw the original error instead of fallback
     ...
     */
    assessRiskLevel(message, sentimentScore) {
     ...
     */
    buildSystemPrompt(userContext, riskLevel) {
     ...
     */
    buildConversationHistory(userContext) {
     ...
     */
    analyzeResponse(response, riskLevel, processingTime) {
        ...
        else if (response.includes('breathing') || response.includes('ground'))
            intervention = 'anxiety_management';
        else if (response.includes('small step') || response.includes('self-care'))
            intervention = 'depression_support';
        else if (response.includes('relationship') || response.includes('communication'))
            intervention = 'relationship_support';
        // Detect personality mode
        let personality = 'adaptive';
        if (response.includes('feel') && response.includes('understand'))
            personality = 'empathetic';
        else if (response.includes('step') && response.includes('plan'))
            personality = 'practical';
        else if (response.includes('breathe') || response.includes('present'))
            personality = 'mindful';
        else if (response.includes('meaning') || response.includes('growth'))
            personality = 'wise';
     ...
     */
    generateFollowUpQuestions(intervention) {
        const followUps = {
            crisis_intervention: [
     ...
     */
    generateSuggestedActions(intervention, riskLevel) {
        if (riskLevel === 'high') {
     ...
     */
    getFallbackResponse(message, userContext, sentimentScore) {
        const riskLevel = this.assessRiskLevel(message, sentimentScore);
...
exports.groqAI = new GroqAIService();
// Helper function for backward compatibility  
const generateGroqResponse = async (message, userId, userContext = {}, sentimentScore) => {
    // Create a fresh instance each time to ensure environment is loaded
    const groqService = new GroqAIService();
    return groqService.generateResponse(message, userId, userContext, sentimentScore);
};
